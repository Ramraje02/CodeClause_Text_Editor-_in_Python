...
# Creating all the functions of all the buttons in the Text-Editor
...
def save_file():
    ...
    if file is None:
        file = fd.asksaveasfilename(initialfile='Untitled.txt', defaultextension='.txt',
                                    filetypes=[("Text File", "*.txt*"), ("Word Document", '*,docx*'), ("PDF",
                                                                                                       "*.pdf*")])
    else:
        file = open(file, "w")
        file.write(text_area.get(1.0, END))
...
def change_font_size():
    text_area.event_generate("<<Font>>")


def about_notepad():
    mb.showinfo("About Notepad", "This is one of the best version of text editor")
...
edit_menu.add_command(label='Font', command=change_font_size)
